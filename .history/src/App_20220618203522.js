...
function App() {
  ...
  const [movChecked, setMovChecked] = useState(true);
...
function OpChange(){
  return(
    ...
    <Labels>
      <CheckWrapper>
        MOV
        <OpBox type="checkbox"  checked={movChecked} onClick={(mov) => HandleCheck(mov)} />
      </CheckWrapper>
      <CheckWrapper>
        XCHG
        <OpBox type="checkbox" value={xchgChecked} onClick={(xchg) => HandleCheck(xchg)}/>
      </CheckWrapper>
    </Labels>
    </>
    )
