...
function Compile(){
  ...
  console.log(movChecked)
  console.log(checkCurrent)
  if(setMovChecked==true && checkCurrent==1 && text.topText!=""){
    
    
    if(checkedBoxes[0]){
      console.log("here")
      if(text.topText.length<=2){tempArray[1]=text.topText; setRegValues(tempArray)}
      if(text.topText.length<=4){tempArray[1]=text.topText[1]+text.topText[0]; tempArray[0]=text.topText.substring(2); setRegValues(tempArray)}
    }
  ...
  else if(setXchgChecked==true && checkCurrent==2 && text.topText!=""){
