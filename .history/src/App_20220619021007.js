...
function Compile(){
  ...
  if(movChecked==true && checkCurrent==1 && text.topText!=""){
    ...
    if(checkedBoxes[0]){
      ...
      else if(text.topText.length<=4){tempArray[1]=text.topText[1]+text.topText[0]; tempArray[0]=text.topText.substring(2); setRegValues(tempArray);}
    }
    else if(checkedBoxes[1]){
      if(text.topText.length<=2){tempArray[3]=text.topText; setRegValues(tempArray)}
      else if(text.topText.length<=4){tempArray[3]=text.topText[1]+text.topText[0]; tempArray[2]=text.topText.substring(2); setRegValues(tempArray)}
    }
    else if(checkedBoxes[2]){
      if(text.topText.length<=2){tempArray[5]=text.topText; setRegValues(tempArray)}
      else if(text.topText.length<=4){tempArray[5]=text.topText[1]+text.topText[0]; tempArray[4]=text.topText.substring(2); setRegValues(tempArray)}
    }
    else if(checkedBoxes[3]){
      if(text.topText.length<=2){tempArray[7]=text.topText; setRegValues(tempArray)}
      else if(text.topText.length<=4){tempArray[7]=text.topText[1]+text.topText[0]; tempArray[6]=text.topText.substring(2); setRegValues(tempArray)}
    }
  }
  
  else if(xchgChecked==true && checkCurrent==2){
    console.log("here")
    let firstBox=null;
    let secondBox=null;
    let tempNum;
    if (checkedBoxes[4]){secondBox=0; firstBox=1}
    else if(checkedBoxes[5]){secondBox=2; firstBox=3}
    else if(checkedBoxes[6]){secondBox=4; firstBox=5}
    else if(checkedBoxes[7]){secondBox=6; firstBox=7}
    console.log(secondBox);
    if(checkedBoxes[0]){
      tempNum=regValues[0];
      tempArray[0]=tempArray[secondBox]
      tempArray[secondBox]=tempNum
      tempNum=regValues[1];
      tempArray[1]=tempArray[firstBox]
      tempArray[firstBox]=tempNum
      setRegValues(tempArray)
    }
    if(checkedBoxes[1]){
      tempNum=regValues[2];
      tempArray[2]=tempArray[secondBox]
      tempArray[secondBox]=tempNum
      tempNum=regValues[3];
      tempArray[3]=tempArray[firstBox]
      tempArray[firstBox]=tempNum
      setRegValues(tempArray)
    ...
    if(checkedBoxes[2]){
      tempNum=regValues[4];
      tempArray[4]=tempArray[secondBox]
      tempArray[secondBox]=tempNum
      tempNum=regValues[5];
      tempArray[5]=tempArray[firstBox]
      tempArray[firstBox]=tempNum
      setRegValues(tempArray)
    }
    if(checkedBoxes[3]){
      tempNum=regValues[6];
      tempArray[6]=tempArray[secondBox]
      tempArray[secondBox]=tempNum
      tempNum=regValues[7];
      tempArray[7]=tempArray[firstBox]
      tempArray[firstBox]=tempNum
      setRegValues(tempArray)
    }
...
const [regValues,setRegValues] = useState(["00","00","00","00","00","00","00","66"])
      ...
      <UiWindow>
        <UiWrapper>
          ...
          <SettingsWrapper>
              ...
              <LineWrap>
              <NumDesc textContent={"Value in hex"}/>  <RegInput type="text" id="field-1" className="field" name="topText" onChange={handleChange}  maxLength={4} style={{width:"40px"}}  value={text.topText}/> 
              
              </LineWrap>
              <Run onClick={Compile}>COMPILE</Run>
              <Run onClick={() => HandleCheck(99)}>RUN</Run>
          </SettingsWrapper>
